    ...
    public class HabitRepository : IHabitRepository
    {
        ...
        public async Task<List<SummaryDTO>> GetSummary(string userId, int year)
        {
            var days = await _context.Days
                ...
                .Where(d => d.DayHabits!.Any(dh => dh.Habit != null && dh.Habit.UserId == userId) && d.Date.HasValue && d.Date.Value.Year == year)
                .ToListAsync();
            ...
            var summary = days.Select(d => new SummaryDTO
            {
                Id = d.Id,
                Date = d.Date,
                Completed = d.DayHabits!.Count(dh => dh.Habit != null && dh.Habit.UserId == userId),
